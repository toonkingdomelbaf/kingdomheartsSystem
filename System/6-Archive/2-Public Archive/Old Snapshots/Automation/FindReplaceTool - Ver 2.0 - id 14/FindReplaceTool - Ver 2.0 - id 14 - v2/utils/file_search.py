...
def run_search_in_path(gui, path, search_text, selected_type="file", 
    ...
    # gui.text_results.delete("1.0", "end")  # COMMENT OUT THIS LINE to append results
    ...
    def process_file(file_path):
        nonlocal total_finds, files_with_finds

        lines = []
        primary_line_count = 0  # Track how many lines are "primary" (paragraphs/tables) vs runs
        ...
        try:
            ...
            if ext.lower() == ".txt":
                with open(file_path, "r", encoding="utf-8") as f:
                    lines = [line.rstrip("\n") for line in f]
                primary_line_count = len(lines)  # All lines are primary for txt

            elif ext.lower() == ".docx":
                doc = Document(file_path)
                primary_lines = []

                # ----- BODY TEXT (non-heading paragraphs) -----
                if content_type in ("all", "text"):
                    for p in doc.paragraphs:
                        if p.text.strip():
                            if content_type == "text":
                                if p.style and not p.style.name.startswith("Heading"):
                ...
                primary_line_count = len(lines)  # All lines are primary for pdf

            else:
                if not silent_mode:
                    start = gui.text_results.index("end-1c")
                    gui.text_results.insert(
                        "end", f"Unsupported file type: {ext} ({file_path})\n\n"
                    )
            ...
            if not silent_mode:
                start = gui.text_results.index("end-1c")
                gui.text_results.insert(
                    "end", f"Error reading file: {file_path} -> {e}\n\n"
                )
                end = gui.text_results.index("end-1c")
                gui.text_results.tag_add("bold", start, end)
            return

        # --- SEARCH LINES --- (for runs and paragraphs)
        for i, line in enumerate(lines, start=1):
            matches = []

            # Use word boundaries to match the whole word (optional, based on user needs)
            if use_regex:
                # Add \b for word boundaries if necessary
                pattern = r"\b" + re.escape(search_text) + r"\b" if not case_sensitive else search_text
                try:
                    matches = [m.span() for m in re.finditer(pattern, line, flags)]
                except re.error:
                    continue
            else:
            ...
            if matches:
                # Only count matches from primary lines (paragraphs/tables) to match replacement count
                if i <= primary_line_count:
                    file_find_count += len(matches)
                    total_finds += len(matches)
                match_lines.append((i, line, matches))
        ...
        if file_find_count > 0:
            ...
            start = gui.text_results.index("end-1c")
            gui.text_results.insert(
                "end", f"{file_path} ({file_find_count} matches)\n"
            )
            end = gui.text_results.index("end-1c")
            gui.text_results.tag_add("bold", start, end)

            for line_no, text, matches in match_lines:
            ...
            # Adding extra line space after each result
    ...
    gui.text_results.insert(
        "end",
        f"Search complete.\n"
        f"Files with matches: {files_with_finds}\n"
        f"Total matches found: {total_finds}\n"
    )

    # Adding extra line space after the summary
    gui.text_results.insert("end", "\n")  # <-- Add this line
