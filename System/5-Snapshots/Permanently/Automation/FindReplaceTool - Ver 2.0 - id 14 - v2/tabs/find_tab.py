...
class FindTab:
    ...
    def apply_remove_line(self):
        ...
        # Search files/folders for matches
        ...
        for result in results:
            ...
            try:
                if ext == ".txt":
                    ...
                    # Remove blank lines
                    removed_count = remove_blank_line_at_matches(
                        doc_or_text=file_path,
                        matches_list=matches,
                        position=position
                    )

                    # Save only if lines removed
                    if removed_count > 0:
                        # Re-read DOCX-style blank removal for TXT
                        # For TXT, we manually remove one empty line after/before match
                        for match_index, match_text, _ in matches:
                            for i, line in enumerate(text):
                                if match_text.strip() == line.strip():
                                    idx = i + 1 if position == "after" else i - 1
                                    if 0 <= idx < len(text) and text[idx].strip() == "":
                                        text.pop(idx)
                                        break
        ...
        if not per_file_removed:
            ...
            for file_path, count in per_file_removed:
                gui.text_results.insert("end", f"{file_path} ({count} blank lines removed)\n")
        gui.text_results.config(state="disabled")
        ...
        # Update status
        if total_removed > 0:
            gui.status_label.config(text=f"Blank lines removed: {total_removed}", fg="green")
        else:
